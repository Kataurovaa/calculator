...
double complex schet(int arr[], int a, int indpoint)
{
    double complex sum, q;
    int s, b, b1;
    sum = 0;
    s = 0;
    q = 1;
...
double complex r(double complex a[], int o, char znak)
{
    float cons;
    switch (znak)
    {
...
double complex find(var var[], char per[], int lenght) {
    int f = 0, j = 0, comp = 0;
    while (f == 0)
    {
...
double complex func(char f, double complex value) {
    if (f == 'c')
        return ccos(value); //+
    if (f == 's')
        return csin(value); //+
    if (f == 'n')
        return  clog(value); //+ lng
    if (f == 'l')
        return clog10(value); //+
    if (f == 'a')
        return cabsf(value); //
            ...
            if (metka == 1)
            {
                for (int m = 0; m<15; m++)
                {
                    if (str2[m] >= 'a' && str2[m] <= 'z' && str2[m + 1] == '(')
                    {
                        ...
                        if (str2[m - 1] == 'q' || str2[m - 1] == 'n' || str2[m - 1] == 'h')
                            f = str2[m - 1];
                        else
                            f = str2[m - 2];
                        m = j;
...
void parser(var* variables) {
    ...
    if (mar == 0) {
        variables->val = atof(variables->formula) + 0*I;
    }
}
...
{
    var variables[SIZE];
    char formula[SIZE]; char out[SIZE];
    FILE* inp = fopen("C:/Server/data/htdocs/calculator/uploades/1.txt", "r");
    FILE* outp = fopen("C:/Server/data/htdocs/calculator/calc/cmake-build-debug/out.txt", "w");
    int n;
    ...
    variables[2].name[0] = 'j';
    variables[2].val = 0 + 1*I;
    fscanf(inp, "%d", &n);
    fscanf(inp, "%s = ", formula);
    printf("%s = ", formula);
    fprintf(outp, "%s = ", formula);
    for (int i = 0; i < n - 1; i++) {
        fscanf(inp, "%s = %s", variables[i+3].name, variables[i+3].formula);
        parser(&variables[i+3]);
    }
    for (int i = n +1 ; i >= 2; i--) {
    ...
    double complex itog = rpn(strcat(formula, " "), variables);
    if (itog == -0)
        itog = 0;
    fprintf(outp,"%lf", creal(itog));
    printf("%lf", creal(itog));
    if (!(cimag(itog) >= -0.00003 && cimag(itog) <= 0.00003))
    {
        fprintf(outp," + %lf*j", cimag(itog));
        printf(" + %lf*j", cimag(itog));
    }
    for (int i=0; i< n-1; ++i)
    {
        fprintf(outp, "\n%s = ", variables[i+3].name);
        printf("\n%s = ", variables[i+3].name);
        if (variables[i+3].mar == 1)
        {
            fprintf(outp, "%s = ",  variables[i+3].formula);
            printf("%s = ",  variables[i+3].formula);
        }
        fprintf(outp, "%lf",  creal(variables[i+3].val));
        printf("%lf",  creal(variables[i+3].val));
        if (!(cimag(variables[i+3].val) >= -0.00003 && cimag(variables[i+3].val) <= 0.00003))
        {
            if (cimag(variables[i+3].val) > 0)
            {
                fprintf(outp, " + ");
                printf(" + ");
            }
            fprintf(outp, "%lf*j",  cimag(variables[i+3].val));
            printf("%lf*j",  cimag(variables[i+3].val));
        }
    }
}
